...
public function show(Request $request, $id, $day)
{
    ...
    $service = Service::query()
        ->where('id', $id)
        ->with([
            'stylists' => function ($q) use ($day, $current_date) {
                // Filtrar los estilistas por el día de la semana
                $q->where('working_days', 'like', "%$day%");
            },
            'stylists.appointments' => function ($q) use ($current_date) {
                // Cargar las citas de los estilistas filtrados por fecha
